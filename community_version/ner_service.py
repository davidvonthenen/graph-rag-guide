...
CALL {
    ...
    UNWIND CASE WHEN size(paragraphs) > 0 THEN paragraphs ELSE [NULL] END AS para
        ...
        CALL {
            WITH p, para
            WITH p, para WHERE para.doc_uuid IS NOT NULL
            MATCH (d:Document {doc_uuid:para.doc_uuid})
            MERGE (p)-[:PART_OF]->(d)
            RETURN count(*) AS __
        }
...
def _normalize_entity(node: Dict[str, Any] | None) -> Optional[Dict[str, Any]]:
    if not node:
        return None
    ent_uuid = _node_get(node, "ent_uuid") or str(uuid.uuid4())
...
def _entity_exists_in_cache(sess: Session, entity: Dict[str, Any]) -> bool:
    ent_uuid = entity.get("ent_uuid")
    if ent_uuid:
        record = sess.run(
            """
            MATCH (e:Entity {ent_uuid:$uuid})
            RETURN e.ent_uuid AS ent_uuid
            LIMIT 1
            """,
            uuid=ent_uuid,
...
def _promote_entity(
    ...
    entity_payload = _normalize_entity(records[0].get("entity"))
    if not entity_payload:
        return False

    if _entity_exists_in_cache(short_sess, entity_payload):
        # Refresh the TTL on the entity but avoid redundant paragraph/document writes.
        _write_promotion_to_cache(short_sess, entity_payload, [], [], exp_ts)
        return False

    documents: Dict[str, Dict[str, Any]] = {}
    paragraphs: Dict[Tuple[Any, Any, Any], Dict[str, Any]] = {}

    for rec in records:
        doc_payload = _normalize_document(rec.get("doc"))
        if doc_payload:
            documents[doc_payload["doc_uuid"]] = doc_payload

        for para_payload in _normalize_paragraphs(rec.get("paras")):
            key = (
                para_payload.get("para_uuid"),
                para_payload.get("doc_uuid"),
                para_payload.get("index"),
            )
            if key not in paragraphs:
                paragraphs[key] = para_payload
